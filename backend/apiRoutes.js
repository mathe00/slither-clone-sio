...
function createApiRouter(dependencies) {
  ...
  router.get('/admin/settings', requireAdmin, (req, res) => {
    try {
      const settingsToSend = {
        ...
        MAX_TRAIL_LENGTH_DEFAULT: config.MAX_TRAIL_LENGTH_DEFAULT,
        INITIAL_SIZE: config.INITIAL_SIZE,
        MIN_BOOST_SIZE: config.MIN_BOOST_SIZE,
        SPEED: config.SPEED,
        MAX_ROTATION_RATE: config.MAX_ROTATION_RATE,
        BOOST_CONSUMPTION_RATE: config.BOOST_CONSUMPTION_RATE,
        FOOD_SPAWN_INTERVAL: config.FOOD_SPAWN_INTERVAL,
        FPS: config.FPS,
        FOOD_ATTRACTION_STRENGTH: config.FOOD_ATTRACTION_STRENGTH,
        FOOD_SNAP_DISTANCE: config.FOOD_SNAP_DISTANCE,
        TELEPORT_VIOLATION_THRESHOLD: config.TELEPORT_VIOLATION_THRESHOLD,
        TELEPORT_TOLERANCE_FACTOR: config.TELEPORT_TOLERANCE_FACTOR,
        DEFAULT_ZOOM_LEVEL: config.DEFAULT_ZOOM_LEVEL,
        BOTS_ENABLED: config.BOTS_ENABLED,
        BOT_SPAWN_INTERVAL_MS: config.BOT_SPAWN_INTERVAL_MS,
        BOT_MAX_COUNT: config.BOT_MAX_COUNT,
        BOT_EXPIRATION_TIME_MS: config.BOT_EXPIRATION_TIME_MS,
  ...
  router.post('/admin/update', requireAdmin, (req, res) => {
    ...
    const keysToUpdate = [
      ...
      'foodSnapDistance',
      'teleportViolationThreshold',
      'teleportToleranceFactor',
      'defaultZoomLevel',
      'botsEnabled',
      'botSpawnIntervalMs',
      'botMaxCount',
    ...
    keysToUpdate.forEach(key => {
      ...
      if (req.body[key] !== undefined && req.body[key] !== '' && config.hasOwnProperty(configKey)) {
        ...
        try {
          // Encapsulate conversion and comparison in a try...catch
          // --- Type Conversion ---
          if (typeof originalValue === 'boolean') {
            ...
            if (!isNaN(num)) {
              ...
              console.warn(
                `Admin: Invalid number format for ${configKey}: '${submittedHttpValue}'. Skipping update for this key.`
              );
            ...
            console.warn(
              `Admin: Unsupported original type for ${configKey} ('${typeof originalValue}'). Skipping update for this key.`
            );
          ...
          if (
            configKey === 'SECURITY_MODE' &&
            !['high', 'medium', 'low'].includes(convertedFormValue)
          ) {
            console.warn(
              `Admin: Invalid value for SECURITY_MODE: '${convertedFormValue}'. Must be 'high', 'medium', or 'low'.`
            );
            return;
          }
          if (
            configKey === 'BOT_DIFFICULTY' &&
            !['easy', 'medium', 'hard'].includes(convertedFormValue)
          ) {
            console.warn(
              `Admin: Invalid value for BOT_DIFFICULTY: '${convertedFormValue}'. Must be 'easy', 'medium', or 'hard'.`
            );
            return;
          }
          if (configKey === 'MAP_SHAPE' && !['rectangle', 'circle'].includes(convertedFormValue)) {
            console.warn(
              `Admin: Invalid value for MAP_SHAPE: '${convertedFormValue}'. Must be 'rectangle' or 'circle'.`
            );
            return;
          }
          ...
          if (originalValue !== convertedFormValue) {
            ...
            if (
              ['MAP_WIDTH', 'MAP_HEIGHT', 'MAP_SHAPE', 'SECURITY_MODE', 'AOI_RADIUS'].includes(
                configKey
              )
            )
            ...
            if (configKey === 'FPS' && !restartRequired) {
              if (
                ...
                console.warn('GameLogic methods not available for FPS update.');
              }
            }
            if (configKey === 'DEFAULT_ZOOM_LEVEL') {
              io.emit('configUpdate', { zoomLevel: convertedFormValue });
              console.log('Zoom level updated, notifying clients.');
            }
            if (
              configKey.startsWith('BOT_') &&
              typeof GameLogic !== 'undefined' &&
              typeof GameLogic.updateBotSettings === 'function'
            ) {
          ...
          console.error(
            `Admin: Error processing key ${key} (${configKey}) with value '${submittedHttpValue}':`,
            error
          );
        ...
        console.warn(
          `Admin: Received key ${key} (maps to ${configKey}) which does not exist in current config. Skipping.`
        );
      }
    });
    ...
    if (req.body.adminMessageText !== undefined) {
      ...
      if (
        ...
        (newMessage && dependencies.adminMessage.duration !== newDuration)
      ) {
        ...
        configChanged = true;
        console.log(
          `Admin message: "${dependencies.adminMessage.text}" (Duration: ${newDuration}ms)`
        );
      }
    }
    ...
    let messageOptions = {};
    if (restartRequired)
      messageKey = 'feedback.settingsUpdatedWithRestart'; // New key needed
  ...
  router.post('/admin/users/:username/setAdmin', requireAdmin, (req, res) => {
    ...
    if (typeof isAdmin !== 'boolean')
      return res.status(400).json({ success: false, messageKey: 'api.errors.invalidAdminStatus' }); // Use key
    if (!accounts[targetUsername] || accounts[targetUsername].isTemporary)
      return res.status(404).json({ success: false, messageKey: 'api.errors.userNotFound' }); // Use key

    accounts[targetUsername].isAdmin = isAdmin;
    saveAccounts();
    console.log(
      `Admin: Admin status of ${targetUsername} updated to ${isAdmin} by ${req.user.username}`
    );
    res.json({
      success: true,
      messageKey: 'api.admin.adminStatusUpdated',
      messageOptions: { username: targetUsername },
    }); // Use key
  });
  ...
  router.post('/admin/users/:username/setSuspended', requireAdmin, (req, res) => {
    ...
    if (typeof isSuspended !== 'boolean')
      return res
        .status(400)
        .json({ success: false, messageKey: 'api.errors.invalidSuspendStatus' }); // Use key
    if (!accounts[targetUsername] || accounts[targetUsername].isTemporary)
      return res.status(404).json({ success: false, messageKey: 'api.errors.userNotFound' }); // Use key
    ...
    if (isSuspended) {
      ...
      if (sessionsRemoved > 0) {
        saveSessions();
        console.log(
          `Admin: ${sessionsRemoved} active session(s) invalidated for ${targetUsername} (suspended).`
        );
      }
    ...
    console.log(
      `Admin: Suspension status of ${targetUsername} updated to ${isSuspended} by ${req.user.username}`
    );
    res.json({
      success: true,
      messageKey: 'api.admin.suspendStatusUpdated',
      messageOptions: { username: targetUsername },
    }); // Use key
  });
  ...
  router.delete('/admin/users/:username', requireAdmin, (req, res) => {
    ...
    if (req.user.username === targetUsername)
      return res.status(403).json({ success: false, messageKey: 'api.errors.cannotDeleteSelf' }); // Use key
    ...
    res.json({
      success: true,
      messageKey: 'api.admin.userDeleted',
      messageOptions: { username: targetUsername },
    }); // Use key
  });
  ...
  router.delete('/admin/users/prune', requireAdmin, (req, res) => {
    ...
    try {
      ...
      if (usersToDelete.length === 0)
        return res.json({ success: true, messageKey: 'api.admin.pruneNoUsers', prunedCount: 0 }); // Use key
      ...
      res.json({
        success: true,
        messageKey: 'api.admin.pruneSuccess',
        messageOptions: { count: prunedCount },
  ...
  router.get('/check-auth', authenticateUser, (req, res) => {
    ...
    res.json({
      ...
      headColor: req.user?.headColor,
      bodyColor: req.user?.bodyColor,
      skinData: req.user?.skinData,
    });
  });
  ...
  router.post('/createAccount', async (req, res) => {
    ...
    if (now - ipRateLimit[ip].firstTimestamp > ACCOUNT_CREATION_TIMEFRAME)
      ipRateLimit[ip] = { count: 0, firstTimestamp: now };
    if (ipRateLimit[ip].count >= ACCOUNT_CREATION_LIMIT) {
      ...
      return res
        ...
        .json({
          success: false,
          messageKey: 'api.errors.rateLimit',
          messageOptions: { minutes: minutesLeft },
        }); // Use key
    }

    if (!username || !password)
      return res.status(400).json({ success: false, messageKey: 'api.errors.missingCredentials' }); // Use key
    if (username.length > MAX_USERNAME_LENGTH)
      return res
        .status(400)
        .json({
          success: false,
          messageKey: 'api.errors.usernameTooLong',
          messageOptions: { maxLength: MAX_USERNAME_LENGTH },
        }); // Use key
    if (username.length < 3)
      return res
        ...
        .json({
          success: false,
          messageKey: 'api.errors.usernameTooShort',
          messageOptions: { minLength: 3 },
        }); // Use key
    if (password.length < 4)
      return res
        ...
        .json({
          success: false,
          messageKey: 'api.errors.passwordTooShort',
          messageOptions: { minLength: 4 },
        }); // Use key
    ...
    for (let existingUsername in accounts) {
      if (
        !accounts[existingUsername].isTemporary &&
        existingUsername
          .toLowerCase()
          .normalize('NFD')
          .replace(/[\u0300-\u036f]/g, '') === normalizedUsername
      ) {
  ...
  router.post('/login', async (req, res) => {
    ...
    if (!username || !password)
      return res.status(400).json({ success: false, messageKey: 'api.errors.missingCredentials' }); // Use key
    const account = accounts[username];
    if (!account || account.isTemporary)
      return res.status(401).json({ success: false, messageKey: 'api.errors.invalidCredentials' }); // Use key
  ...
  router.post('/updateSkin', authenticateUser, (req, res) => {
    ...
    if (!req.user)
      return res.status(401).json({ success: false, messageKey: 'api.errors.notAuthenticated' }); // Use key
    const { headColor, skinData } = req.body;
    const username = req.user.username;

    if (!headColor || !/^#[0-9a-fA-F]{6}$/.test(headColor)) {
      console.error(`Validation failed for ${username}: Invalid headColor "${headColor}"`);
      return res.status(400).json({ success: false, messageKey: 'api.errors.invalidHeadColor' });
    } // Use key
    if (!skinData || typeof skinData !== 'object' || skinData === null) {
      console.error(
        `Validation failed for ${username}: skinData is missing or not an object. Received:`,
        skinData
      );
      return res.status(400).json({ success: false, messageKey: 'api.errors.missingSkinData' });
    } // Use key
    if (!['single', 'pattern'].includes(skinData.bodyType)) {
      console.error(`Validation failed for ${username}: Invalid bodyType "${skinData.bodyType}"`);
      return res.status(400).json({ success: false, messageKey: 'api.errors.invalidBodyType' });
    } // Use key
    if (!skinData.bodyColor || !/^#[0-9a-fA-F]{6}$/.test(skinData.bodyColor)) {
      console.error(
        `Validation failed for ${username}: Invalid bodyColor "${skinData.bodyColor}" in skinData`
      );
      return res.status(400).json({ success: false, messageKey: 'api.errors.invalidBodyColor' });
    } // Use key
    if (!Array.isArray(skinData.patternColors)) {
      console.error(
        `Validation failed for ${username}: patternColors is not an array. Received:`,
        skinData.patternColors
      );
      return res
        .status(400)
        .json({ success: false, messageKey: 'api.errors.invalidPatternColorsType' });
    } // Use key
    if (skinData.patternColors.length > 8) {
      console.error(
        `Validation failed for ${username}: Too many patternColors (${skinData.patternColors.length})`
      );
      return res
        .status(400)
        .json({
          success: false,
          messageKey: 'api.errors.tooManyPatternColors',
          messageOptions: { max: 8 },
        });
    } // Use key
    if (!skinData.patternColors.every(c => typeof c === 'string' && /^#[0-9a-fA-F]{6}$/.test(c))) {
      console.error(
        `Validation failed for ${username}: Invalid color found in patternColors. Received:`,
        skinData.patternColors
      );
      return res
        .status(400)
        .json({ success: false, messageKey: 'api.errors.invalidPatternColorValue' });
    } // Use key
    ...
    if (!skinData.trailEffect || !allowedEffects.includes(skinData.trailEffect)) {
      console.error(
        `Validation failed for ${username}: Invalid trailEffect "${skinData.trailEffect}"`
      );
      return res.status(400).json({ success: false, messageKey: 'api.errors.invalidTrailEffect' });
    } // Use key
  ...
  router.delete('/deleteAccount', authenticateUser, (req, res) => {
    if (!req.user)
      return res.status(401).json({ success: false, messageKey: 'api.errors.notAuthenticated' }); // Use key
    const username = req.user.username;
    if (!accounts[username] || accounts[username].isTemporary)
      return res.status(404).json({ success: false, messageKey: 'api.errors.userNotFound' }); // Use key
    if (accounts[username].isAdmin)
      return res
        .status(403)
        .json({ success: false, messageKey: 'api.errors.adminCannotDeleteSelf' }); // Use key
