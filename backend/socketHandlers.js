...
function init(deps) {
  ...
  getLeaderboard = deps.getLeaderboard;
  saveAccounts = deps.saveAccounts;
  saveSessions = deps.saveSessions;
  gameMode = deps.gameMode;
  checkFoodSpawning = deps.checkFoodSpawning;
  initialMapWidth = deps.initialMapWidth;
...
function setupSocketListeners() {
  io.on('connection', socket => {
      ...
      if (cookieHeader) {
        ...
        if (initialToken && sessions[initialToken] && sessions[initialToken].expires > Date.now()) {
          ...
          if (accounts[username] && !accounts[username].isSuspended) {
            ...
            console.log(
              `Socket ${socket.id} authenticated via cookie: ${authenticatedUsername} (Admin: ${isAdmin})`
            );
          } else
            console.log(`Session cookie found for ${username} but account missing or suspended.`);
        }
      }
    } catch (e) {
      console.error('Error reading initial socket cookie:', e);
    }
    ...
    socket.on('joinGame', data => {
      ...
      if (socket.username) {
        // Authenticated user joining
        displayName = socket.username;
        isTemporary = false;
        const account = accounts[displayName];
        if (!account) {
          socket.emit('loginFailed', { messageKey: 'socket.loginFailed.accountError' });
          socket.disconnect(true);
          return;
        }
        if (account.isSuspended) {
          socket.emit('loginFailed', { messageKey: 'socket.loginFailed.suspended' });
          socket.disconnect(true);
          return;
        }
        if (joinAsGhost && !socket.isAdmin) {
          console.warn(`Non-admin user ${displayName} attempted to join as ghost.`);
          joinAsGhost = false;
        }
        ...
        console.log(
          `Authenticated player ${displayName} joining via ${socket.id} (Ghost: ${joinAsGhost}).`
        );
      } else {
        // Guest player joining
        displayName = `Slither${Math.floor(100000 + Math.random() * 900000)}`;
        isTemporary = true;
        joinAsGhost = false; // Guests cannot be ghosts
      ...
      if (!isTemporary && !joinAsGhost) {
        for (let id in players) {
          if (
            id !== socket.id &&
            players[id].name === displayName &&
            !players[id].isTemporary &&
            !players[id].isGhost
          ) {
      ...
      if (!spawnLocation) {
        console.error('FATAL: Could not determine any spawn location!');
        socket.disconnect(true);
        return;
      }
      const startX = spawnLocation.x;
      const startY = spawnLocation.y;
      ...
      players[socket.id] = {
        ...
        headColor,
        bodyColor,
        skinData, // Store determined colors/skin
        x: startX,
        y: startY,
        angle: Math.random() * 2 * Math.PI,
        mouseX: startX,
        mouseY: startY, // Initial mouse position
        ...
        boost: false,
        boostTick: 0,
        maxTrailLength: joinAsGhost ? 0 : config.INITIAL_SIZE,
        killCount: 0,
        cheatStart: null,
        ...
        isTemporary,
        isAdmin: socket.isAdmin || false, // Use stored admin status
        isGhost: joinAsGhost,
        isFrozen: false,
        lastMouseMove: Date.now(),
        type: 'player', // Mark as human player
        lastPosition: { x: startX, y: startY, time: Date.now() }, // For anti-cheat
        teleportViolations: 0,
        isPanning: false,
        panStartX: 0,
        panStartY: 0, // For ghost panning
        panStartCameraX: startX,
        panStartCameraY: startY,
        cameraX: startX,
        cameraY: startY, // Initial camera for ghost
      ...
      const mapRadius =
        config.MAP_SHAPE === 'circle' ? Math.min(initialMapWidth, initialMapHeight) / 2 : 0;
      ...
      if (
        !isTemporary &&
        !joinAsGhost &&
        (!lastNotificationTimes[playerUsername] ||
          now - lastNotificationTimes[playerUsername] > notificationCooldown)
      ) {
        socket.broadcast
          .compress(true)
          .emit('userConnected', {
            messageKey: 'socket.userConnected',
            messageOptions: { username: playerUsername },
          });
    ...
    socket.on('startPan', data => {
      ...
      if (p && p.isGhost && typeof data.x === 'number' && typeof data.y === 'number') {
        ...
        p.panStartX = data.x;
        p.panStartY = data.y;
        p.panStartCameraX = p.cameraX;
        p.panStartCameraY = p.cameraY;
      }
    });
    socket.on('doPan', () => {
    ...
    socket.on('endPan', () => {
      const p = players[socket.id];
      if (p && p.isGhost) p.isPanning = false;
    });
    ...
    socket.on('secretGrow', () => {
      const p = players[socket.id];
      if (p && !p.isGhost && socket.isAdmin) p.maxTrailLength += 10;
    });

    // --- Admin Action Handler ---
    socket.on('adminAction', data => {
      if (!socket.isAdmin) {
        // Check admin status from socket
        console.warn(`Socket ${socket.id} (non-admin) attempted admin action.`);
        socket.emit('adminActionFeedback', {
          success: false,
          messageKey: 'socket.adminFeedback.unauthorized',
        });
        return;
      }
      ...
      if (!targetPlayer) {
        console.warn(
          `Admin ${adminUsername} tried to action non-existent player ${targetPlayerId}.`
        );
        socket.emit('adminActionFeedback', {
          success: false,
          messageKey: 'socket.adminFeedback.targetNotFound',
        });
        return;
      }
      ...
      console.log(
        `Admin ${adminUsername} performing action '${action}' on ${targetUsername} (${targetPlayerId})`
      );
      let feedbackKey = '';
      let feedbackOptions = { username: targetUsername, admin: adminUsername };
      let success = false;
      ...
      switch (action) {
        case 'kick':
          if (targetSocket) {
            targetSocket.emit('kicked', {
              reasonKey: 'socket.kickReason',
              reasonOptions: { admin: adminUsername },
            });
            targetSocket.disconnect(true);
            feedbackKey = 'socket.adminFeedback.kickSuccess';
            success = true;
          } else {
            if (players[targetPlayerId]) {
              ...
              feedbackKey = 'socket.adminFeedback.kickOfflineSuccess';
              success = true;
            } else {
              feedbackKey = 'socket.adminFeedback.kickError';
              success = false;
            }
          }
          break;
        case 'ban':
          if (!targetPlayer.isTemporary && accounts[targetUsername]) {
            accounts[targetUsername].isSuspended = true;
            saveAccounts();
            ...
            if (targetSocket) {
              targetSocket.emit('banned', {
                reasonKey: 'socket.banReason',
                reasonOptions: { admin: adminUsername },
              });
              targetSocket.disconnect(true);
              feedbackKey = 'socket.adminFeedback.banSuccess';
              success = true;
            } else {
              if (players[targetPlayerId]) {
                delete players[targetPlayerId];
              ...
              feedbackKey = 'socket.adminFeedback.banAccountOnlySuccess';
              success = true;
            }
          } else {
            feedbackKey = 'socket.adminFeedback.banError';
            success = false;
          }
          break;
        case 'freeze':
          targetPlayer.isFrozen = !targetPlayer.isFrozen;
          feedbackKey = targetPlayer.isFrozen
            ? 'socket.adminFeedback.freezeSuccess'
            : 'socket.adminFeedback.unfreezeSuccess';
          success = true;
          break;
        case 'kill':
          if (typeof GameLogic.handlePlayerDeath === 'function') {
            GameLogic.handlePlayerDeath(targetPlayerId, null, true, socket.id, {
              reasonKey: 'game.deathReason.killedByAdmin',
              reasonOptions: { admin: adminUsername },
            });
            feedbackKey = 'socket.adminFeedback.killSuccess';
            success = true;
          } else {
            ...
            if (targetSocket) {
              targetSocket.emit('gameOver', {
                ...
                reasonOptions: { admin: adminUsername },
                kill: true,
                finalSize: targetPlayer.maxTrailLength || 0,
              });
              targetSocket.disconnect(true);
            } else if (players[targetPlayerId]) {
              delete players[targetPlayerId];
              if (typeof checkFoodSpawning === 'function') checkFoodSpawning();
              io.emit('leaderboard', getLeaderboard());
            }
            feedbackKey = 'socket.adminFeedback.killSuccess';
            success = true; // Assume success even if simplified
          }
          break;
        case 'clear': // Remove player without spawning food
          if (targetSocket) {
            targetSocket.emit('kicked', {
              reasonKey: 'socket.clearReason',
              reasonOptions: { admin: adminUsername },
            });
            targetSocket.disconnect(true);
          }
          if (players[targetPlayerId]) {
            ...
            if (typeof checkFoodSpawning === 'function') checkFoodSpawning();
            io.emit('leaderboard', getLeaderboard());
            feedbackKey = 'socket.adminFeedback.clearSuccess';
            success = true;
          } else {
            feedbackKey = 'socket.adminFeedback.clearError';
            success = false;
          }
          break;
        default:
          console.warn(`Admin ${adminUsername} sent unknown action: ${action}`);
          feedbackKey = 'socket.adminFeedback.unknownAction';
          success = false;
      }
      socket.emit('adminActionFeedback', {
        success: success,
        messageKey: feedbackKey,
        messageOptions: feedbackOptions,
      });
    });
    ...
    socket.on('clientCollisionDetected', data => {
      ...
      if (config.SECURITY_MODE === 'low') {
        ...
        console.warn(
          `Received clientCollisionDetected from ${socket.id} (Player: ${p.name}, Admin: ${p.isAdmin}) in mode '${config.SECURITY_MODE}'. Ignoring.`
        );
        return;
      }
      ...
      if (collisionTrusted) {
        console.log(
          `Client ${p.name} (${socket.id}) reported collision (trusted in mode '${config.SECURITY_MODE}'${p.isAdmin && config.SECURITY_MODE === 'medium' ? ' as admin' : ''}). Killer ID: ${data?.killerId}`
        );
        if (typeof GameLogic.handlePlayerDeath === 'function') {
          GameLogic.handlePlayerDeath(socket.id, null, true, data?.killerId, {
            reasonKey: 'game.deathReason.clientCollision',
            reasonOptions: {},
          });
        } else {
          console.error(
            'GameLogic.handlePlayerDeath not accessible from socketHandlers for client collision.'
          );
        }
      }
    });
    ...
    socket.on('toggleSelfFreeze', () => {
      if (!socket.isAdmin) {
        console.warn(`Socket ${socket.id} (non-admin) attempted self-freeze.`);
        return;
      }
      const p = players[socket.id];
      if (p && !p.isGhost && p.type === 'player') {
        ...
        console.log(
          `Admin ${p.name} (${socket.id}) ${p.isFrozen ? 'froze' : 'unfroze'} themselves.`
        );
        socket.compress(true).emit('freezeStateUpdate', { isFrozen: p.isFrozen }); // Notify client of their freeze state
      } else
        console.warn(
          `Admin ${socket.username || socket.id} tried to self-freeze but player state invalid.`
        );
    });
    ...
    socket.on('disconnect', reason => {
      ...
      if (
        ...
        accounts[disconnectedUsername] &&
        !accounts[disconnectedUsername].isTemporary
      ) {
        const now = Date.now();
        if (
          !lastNotificationTimes[disconnectedUsername] ||
          now - lastNotificationTimes[disconnectedUsername] > notificationCooldown
        ) {
          // Throttle disconnect notifications
          io.compress(true).emit('userDisconnected', {
            messageKey: 'socket.userDisconnected',
            messageOptions: { username: disconnectedUsername },
          });
