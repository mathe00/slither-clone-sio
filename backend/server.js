  ...
  try {
    if (fs.existsSync(CONFIG_PATH)) {
      ...
      config = {
        ...
        MAP_SHAPE: 'rectangle',
        MAX_TRAIL_LENGTH_DEFAULT: 1000,
        SPEED: 1,
        MAX_ROTATION_RATE: 0.08,
        BOOST_CONSUMPTION_RATE: 1,
        BOOST_CONSUMPTION_INTERVAL: 5,
        MIN_BOOST_SIZE: DEFAULT_MIN_BOOST_SIZE,
        INITIAL_SIZE: DEFAULT_INITIAL_SIZE,
        FOOD_SPAWN_INTERVAL: 5000,
        FOOD_ATTRACTION_RADIUS: 50,
        FOOD_ATTRACTION_STRENGTH: 0.1,
        FOOD_SNAP_DISTANCE: 5,
        FOOD_EXPIRATION_TIME: 10000,
        RANDOMIZATION_FOOD_EXPIRATION_TIME: 3000,
        ...
        GODMODE_DURATION: 5000,
        FPS: 45,
        GRID_CELL_SIZE: 100,
        TELEPORT_VIOLATION_THRESHOLD: 5,
        TELEPORT_TOLERANCE_FACTOR: 1.5,
        DEFAULT_ZOOM_LEVEL: 1.0,
        BOTS_ENABLED: true,
        BOT_SPAWN_INTERVAL_MS: 5000,
        BOT_MAX_COUNT: 10,
        BOT_EXPIRATION_TIME_MS: 120000,
        BOT_DIFFICULTY: 'easy',
        SECURITY_MODE: 'medium',
        AOI_RADIUS: 2000,
        GODMODE_ON_SPAWN_ENABLED: true,
      };
    ...
    if (!['high', 'medium', 'low'].includes(config.SECURITY_MODE)) {
      console.warn(
        `Invalid SECURITY_MODE "${config.SECURITY_MODE}" in config, defaulting to "medium".`
      );
    ...
    console.log(
      `Initial map dimensions: ${initialMapWidth}x${initialMapHeight} (Shape: ${config.MAP_SHAPE})`
    );
  } catch (err) {
...
function saveConfig() {
  try {
    if (!['rectangle', 'circle'].includes(config.MAP_SHAPE)) {
      console.warn(
        `Invalid MAP_SHAPE "${config.MAP_SHAPE}" detected before saving, correcting to "rectangle".`
      );
  ...
  if (fs.existsSync(ACCOUNTS_PATH)) {
    try {
      ...
      for (let username in accounts) {
        if (accounts[username].isTemporary) {
          delete accounts[username];
          updated = true;
        } else {
          if (accounts[username].isAdmin === undefined) {
            accounts[username].isAdmin = false;
            updated = true;
          }
          if (accounts[username].isSuspended === undefined) {
            accounts[username].isSuspended = false;
            updated = true;
          }
          if (accounts[username].headColor === undefined) {
            accounts[username].headColor = '#ff0000';
            updated = true;
          }
          if (accounts[username].bodyColor === undefined) {
            accounts[username].bodyColor = '#ffff00';
            updated = true;
          }
          if (accounts[username].skinData === undefined) {
            accounts[username].skinData = null;
            updated = true;
          }
        }
      }
      ...
      if (updated) {
        console.log('Updated accounts (missing fields/temporary).');
        saveAccounts();
      }
    } catch (err) {
      ...
      for (const token in rawSessions) {
        ...
        if (session.expires > now) {
          ...
          if (accounts[username] && !accounts[username].isSuspended) {
            ...
            console.log(`Session ignored for ${username} (account deleted/suspended).`);
            expiredCount++;
          }
        } else expiredCount++;
      }
  ...
  try {
    ...
    for (const token in sessions) {
      ...
      if (
        ...
        !accounts[session.username].isSuspended
      ) {
  ...
  if (token && sessions[token]) {
    ...
    if (session.expires > Date.now() && accounts[username] && !accounts[username].isSuspended) {
      req.user = {
        ...
        headColor: accounts[username].headColor,
        bodyColor: accounts[username].bodyColor,
      ...
      if (accounts[username] && accounts[username].isSuspended)
        console.log(`Session invalidated for ${username} (account suspended).`);
    }
  }
...
function initializeWorkers() {
  ...
  for (let i = 0; i < numCPUs; i++) {
    ...
    worker.on('exit', code => {
      if (code !== 0) console.error(`Worker ${i} stopped with code ${code}`);
    });
    workers.push(worker);
    workerLoad.push(0);
  }
...
function dispatchToWorker(taskData) {
  if (workers.length === 0) {
    console.warn('No workers available for task.');
    return;
  }
...
function handleWorkerResult(result) {
  ...
  if (error) {
    console.error(`Error returned by worker for ${playerId}: ${error}`);
...
function getLeaderboard() {
  ...
  const inGameUsernames = new Set(
    Object.values(players)
      .filter(p => !p.isGhost && p.type === 'player')
      .map(p => p.name)
  );
  for (const token in sessions) {
    if (sessions[token].expires > now) onlineUsernames.add(sessions[token].username);
  }
...
if (!accounts['admin']) {
  ...
  console.warn(
    '\x1b[33mIt is highly recommended to create an administrator account by running:\x1b[0m'
  );
  console.warn('\x1b[33m  node backend/server.js --set-new-password YOUR_CHOSEN_PASSWORD\x1b[0m');
  console.warn(
    '\x1b[33mWithout an admin account, you will not be able to access the admin panel.\x1b[0m'
  );
  console.warn(
    '\x1b[33mThe server will start, but admin functionalities will be unavailable.\x1b[0m'
  );
  console.warn(
    '\x1b[33m************************************************************************************\x1b[0m\n'
  );
  ...
  console.warn(
    "\x1b[33mWARNING: The 'admin' account exists but seems to have no password set.\x1b[0m"
  );
  console.warn(
    '\x1b[33mPlease ensure you have run the --set-new-password command correctly:\x1b[0m'
  );
  console.warn('\x1b[33m  node backend/server.js --set-new-password YOUR_CHOSEN_PASSWORD\x1b[0m');
  console.warn(
    '\x1b[33m************************************************************************************\x1b[0m\n'
