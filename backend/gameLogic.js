...
function updateStateEmitInterval(processingTime, now) {
  ...
  if (!config.ADAPTIVE_THROTTLING_ENABLED) {
    const targetFPS = Math.max(
      config.MIN_FPS || 10,
      Math.min(config.MAX_FPS || 120, config.FPS || 45)
    );
  ...
  avgProcessingTime =
    processingTimeSamples.reduce((sum, time) => sum + time, 0) / processingTimeSamples.length;

  // Update player count
  playerCount = Object.keys(players).filter(
    id => players[id].type === 'player' && !players[id].isGhost
  ).length;
  ...
  if (now - lastPerformanceCheck > (config.PERFORMANCE_CHECK_INTERVAL || 5000)) {
    ...
    if (playerCount > highLoadThreshold) {
      ...
      const minFPS = Math.max(
        config.MIN_FPS || 10,
        targetFPS * (config.HIGH_LOAD_FPS_REDUCTION || 0.5)
      );
    ...
    if (Math.abs(newInterval - currentStateEmitInterval) > 1) {
      ...
      console.log(
        `Adaptive throttling: Updated state emit interval to ${currentStateEmitInterval.toFixed(2)}ms (FPS: ${(1000 / currentStateEmitInterval).toFixed(1)})`
      );
    }
  }
}
...
function getRandomSpawnLocation() {
  ...
  if (config.MAP_SHAPE === 'circle') {
    ...
    if (radius <= 0) {
      // Handle case where map is too small for margin
    ...
    if (spawnWidth <= 0 || spawnHeight <= 0) {
      console.warn(
        'Map dimensions too small for spawn margin in rectangle mode, spawning at center.'
      );
      return { x: mapCenterX, y: mapCenterY };
    }
...
function findSafeSpawnLocation() {
  ...
  try {
    for (let i = 0; i < NUM_SPAWN_CANDIDATES; i++) {
      ...
      const nearbyPlayers = spatialGrid.queryNearbyRadius(
        candidate.x,
        candidate.y,
        SAFE_SPAWN_RADIUS_PLAYER * 1.2
      );
    ...
    if (bestCandidate && bestScore > BORDER_PENALTY / 2) {
      // Check score is reasonably positive
      console.log(
        `Smart spawn found safe location: (${bestCandidate.x.toFixed(1)}, ${bestCandidate.y.toFixed(1)}) Score: ${bestScore.toFixed(1)}`
      );
...
function handlePlayerDeath(playerId, reason, isKill, killerId = null, deathContext = {}) {
  ...
  if (p.type === 'player' && accounts[p.name]) {
    accounts[p.name].totalSize = (accounts[p.name].totalSize || 0) + (p.maxTrailLength || 0); // Use maxTrailLength
...
function updateBot(p, now, deltaTime) {
  ...
  let diff = Math.atan2(Math.sin(p.targetAngle - p.angle), Math.cos(p.targetAngle - p.angle));
...
function gameLoop() {
  ...
  food = food
    .map(f => {
      ...
      if (now >= f.expirationTime) {
        ...
        f.opacity = Math.max(0, 1 - timeSinceExpiration / config.FADE_OUT_DURATION);
  ...
  for (const id in players) spatialGrid.insert(players[id], players[id].x, players[id].y);
  ...
  for (let id in players) {
    ...
    // --- Normal Player Logic ---
    if (p.godmode && now - p.godmodeStartTime >= config.GODMODE_DURATION) p.godmode = false;
    ...
    if (
      ...
      let diff = Math.atan2(Math.sin(targetAngle - p.angle), Math.cos(targetAngle - p.angle));
      p.angle += Math.max(-config.MAX_ROTATION_RATE, Math.min(config.MAX_ROTATION_RATE, diff));
    }

    const lastTailSegment = p.trail.length > 0 ? p.trail[0] : { x: p.x, y: p.y };
    p.trail.push({ x: p.x, y: p.y });
    ...
    if (deltaTPTime > 0) {
      ...
      if (distMoved > teleportThreshold && distMoved > config.COLLISION_RADIUS * 2) {
      ...
      p.lastPosition = { x: p.x, y: p.y, time: now };
    } else if (p.lastPosition.time !== now) p.lastPosition = { x: p.x, y: p.y, time: now };
    ...
    if (hitBoundary) {
      if (!p.hasCollided) {
        console.log(
          `Boundary collision detected for ${p.name} at (${p.x.toFixed(1)}, ${p.y.toFixed(1)}) Shape: ${config.MAP_SHAPE}`
        );
        p.hasCollided = true;
        // Store key
        p.collisionData = {
    ...
    for (const f of nearbyFoodPlayer) {
      ...
      const attractionRadiusSq = (config.COLLISION_RADIUS + config.FOOD_ATTRACTION_RADIUS) ** 2;
  ...
  if (now - lastStateEmitTime >= currentStateEmitInterval) {
    ...
    for (const playerId in players) {
      ...
      stateForPlayer.players[playerId] = {
        ...
        boost: p.boost,
        godmode: p.godmode,
        godmodeStartTime: p.godmodeStartTime,
        isGhost: p.isGhost,
        isFrozen: p.isFrozen,
        maxTrailLength: p.maxTrailLength,
        type: p.type,
        angle: p.angle, // Include angle
      };
        ...
        if (entity.type === 'player' || entity.type === 'bot') {
          ...
          if (nearbyP) {
            ...
            stateForPlayer.players[entity.id] = {
              ...
              trail: nearbyP.trail,
              boost: nearbyP.boost,
              godmode: nearbyP.godmode,
              godmodeStartTime: nearbyP.godmodeStartTime,
              isGhost: nearbyP.isGhost,
              isFrozen: nearbyP.isFrozen,
              maxTrailLength: nearbyP.maxTrailLength,
              type: nearbyP.type,
              angle: nearbyP.angle, // Include angle
            };
