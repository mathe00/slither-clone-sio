...
const GameRenderer = (() => {
  ...
  // --- Particle Drawing ---
  function drawParticles(gl, shaderPrograms, buffers, projectionMatrix, zoomLevel, gameState, currentTime) {
    ...
    gl.vertexAttribPointer(
      programInfo.attribLocations.position, 2, gl.FLOAT, false, 0, 0
    );
    ...
    for (const id in gameState.players) {
      ...
      for (const particle of p.particles) {
        let r = 1, g = 1, b = 1, a = particle.opacity;
        try {
          if (particle.color.startsWith("rgba")) {
            const parts = particle.color.match(/[\d.]+/g);
            if (parts && parts.length === 4) {
              r = parseFloat(parts[0]) / 255; g = parseFloat(parts[1]) / 255;
              b = parseFloat(parts[2]) / 255; a *= parseFloat(parts[3]);
            }
  ...
  function drawTextOverlay(ctx, canvas, gameState, cameraX, cameraY, zoomLevel, localMyId, selectedPlayerId) {
    ...
    for (let id in gameState.players) {
      ...
      ctx.fillRect(screenX - textWidth / 2 - 4, nameY - fontSize / 2 - 2, textWidth + 8, fontSize + 4);

      // Name text style
      ctx.fillStyle = p.isFrozen ? "rgba(150, 150, 200, 0.9)" : "rgba(255, 255, 255, 0.9)";
      if (id === selectedPlayerId) { // Highlight if selected in ghost mode
  ...
  function drawScreenQuad(gl, shaderPrograms, screenX, screenY, screenWidth, screenHeight, color, projMatrix) {
    const programInfo = shaderPrograms.color;
    ...
    WebGLUtils.translateMatrix(modelMatrix, modelMatrix, [screenX + screenWidth / 2, screenY + screenHeight / 2]);
  ...
  // --- Minimap Drawing ---
  function drawMinimapWebGL(gl, canvas, shaderPrograms, buffers, gameState, localMyId, cameraX, cameraY, zoomLevel, worldWidth, worldHeight, mapShape, mapRadius) {
    ...
    drawScreenQuad(gl, shaderPrograms, mapX, mapY, mapSize, mapSize, bgColor, screenProjectionMatrix);
    ...
    if (mapShape === "circle") {
        ...
        drawScreenQuad(gl, shaderPrograms, mapX, mapY, mapSize, borderWidth, borderColor, screenProjectionMatrix); // Top
        drawScreenQuad(gl, shaderPrograms, mapX, mapY + mapSize - borderWidth, mapSize, borderWidth, borderColor, screenProjectionMatrix); // Bottom
        drawScreenQuad(gl, shaderPrograms, mapX, mapY, borderWidth, mapSize, borderColor, screenProjectionMatrix); // Left
        drawScreenQuad(gl, shaderPrograms, mapX + mapSize - borderWidth, mapY, borderWidth, mapSize, borderColor, screenProjectionMatrix); // Right
    } else { // Rectangle
        drawScreenQuad(gl, shaderPrograms, mapX, mapY, mapSize, borderWidth, borderColor, screenProjectionMatrix); // Top
        drawScreenQuad(gl, shaderPrograms, mapX, mapY + mapSize - borderWidth, mapSize, borderWidth, borderColor, screenProjectionMatrix); // Bottom
        drawScreenQuad(gl, shaderPrograms, mapX, mapY, borderWidth, mapSize, borderColor, screenProjectionMatrix); // Left
    ...
    for (let id in gameState.players) {
      ...
      const clampedX = Math.max(mapX + dotSize / 2, Math.min(mapX + mapSize - dotSize / 2, minimapPosX));
      const clampedY = Math.max(mapY + dotSize / 2, Math.min(mapY + mapSize - dotSize / 2, minimapPosY));

      // Determine dot color based on type/state
      let dotColor = p.type === "bot" ? [0.5, 0.5, 0.5, 0.9] // Grey for bots
                     : p.isFrozen ? [0.0, 1.0, 1.0, 0.9] // Cyan for frozen
                     : [1.0, 0.0, 0.0, 0.9]; // Red for other players

      drawScreenQuad(gl, shaderPrograms, clampedX - dotSize / 2, clampedY - dotSize / 2, dotSize, dotSize, dotColor, screenProjectionMatrix);
    }
    ...
    if (self && !self.isGhost) {
      ...
      // Clamp self position
      const selfClampedX = Math.max(mapX + selfDotSize / 2, Math.min(mapX + mapSize - selfDotSize / 2, selfMinimapPosX));
      const selfClampedY = Math.max(mapY + selfDotSize / 2, Math.min(mapY + mapSize - selfDotSize / 2, selfMinimapPosY));

      drawScreenQuad(gl, shaderPrograms, selfClampedX - selfDotSize / 2, selfClampedY - selfDotSize / 2, selfDotSize, selfDotSize, selfColor, screenProjectionMatrix);
    }
    ...
    if (clampedViewWidth > 0 && clampedViewHeight > 0) {
      // Top line
      drawScreenQuad(gl, shaderPrograms, clampedViewX, clampedViewY, clampedViewWidth, viewRectLineWidth, viewRectColor, screenProjectionMatrix);
      // Bottom line
      drawScreenQuad(gl, shaderPrograms, clampedViewX, clampedViewBottom - viewRectLineWidth, clampedViewWidth, viewRectLineWidth, viewRectColor, screenProjectionMatrix);
      // Left line
      drawScreenQuad(gl, shaderPrograms, clampedViewX, clampedViewY, viewRectLineWidth, clampedViewHeight, viewRectColor, screenProjectionMatrix);
      // Right line
      drawScreenQuad(gl, shaderPrograms, clampedViewRight - viewRectLineWidth, clampedViewY, viewRectLineWidth, clampedViewHeight, viewRectColor, screenProjectionMatrix);
    }
  }


  // --- Main Drawing Function ---
  function drawGame(gl, canvas, textOverlayCtx, textOverlayCanvas, shaderPrograms, buffers, textures, gameState, cameraX, cameraY, zoomLevel, worldWidth, worldHeight, mapShape, mapRadius, localMyId, selectedPlayerId, currentTime, maybeSpawnParticle, clientCollisionRadius) {
    if (!gl || !canvas) return;
    ...
    if (gridProgramInfo && gridProgramInfo.program && buffers.quad) {
        ...
        gl.uniform4f(gridProgramInfo.uniformLocations.bgColor, bgColor[0], bgColor[1], bgColor[2], bgColor[3]);
        gl.uniform4f(gridProgramInfo.uniformLocations.lineColor, lineColor[0], lineColor[1], lineColor[2], lineColor[3]);
    ...
    const pulsatingAlpha = minAlpha + (alphaRange / 2) * (1 + Math.sin((currentTime / 1000) * pulseSpeed));
    ...
    if (mapShape === "circle" && mapRadius > 0) {
        ...
        if (!buffers.circleBorder) {
          buffers.circleBorder = WebGLUtils.createBuffer(new Float32Array(borderVertices), gl.ARRAY_BUFFER, gl.DYNAMIC_DRAW);
        } else {
        ...
        WebGLUtils.drawQuad(worldWidth / 2, -baseBorderWidth / 2, worldWidth + baseBorderWidth, baseBorderWidth, borderColor); // Bottom edge
        WebGLUtils.drawQuad(worldWidth / 2, worldHeight + baseBorderWidth / 2, worldWidth + baseBorderWidth, baseBorderWidth, borderColor); // Top edge
        WebGLUtils.drawQuad(-baseBorderWidth / 2, worldHeight / 2, baseBorderWidth, worldHeight + baseBorderWidth, borderColor); // Left edge
        WebGLUtils.drawQuad(worldWidth + baseBorderWidth / 2, worldHeight / 2, baseBorderWidth, worldHeight + baseBorderWidth, borderColor); // Right edge
    }
    ...
    if (gameState.food) {
      ...
      if (program && program.program && buffers.quad) {
          ...
          for (let i = 0; i < gameState.food.length; i++) {
            ...
            if (item.x < left - 50 || item.x > right + 50 || item.y < bottom - 50 || item.y > top + 50) continue;

            // Calculate pulsating scale and alpha
            const foodAnimTime = (currentTime / 1000) * foodAnimSpeed + (calculateHashCode(item.id || "food") % 10); // Add hash for variation
            ...
            WebGLUtils.scaleMatrix(modelMatrix, modelMatrix, [item.size * foodScale, item.size * foodScale]);
    ...
    if (gameState.players) {
      ...
      drawParticles(gl, shaderPrograms, buffers, projectionMatrix, zoomLevel, gameState, currentTime);
          ...
          for (let id in gameState.players) {
            ...
            if (p.godmode) {
                ...
                if (remainingRatio > 0) {
                    ...
                    const baseAlpha = GODMODE_MIN_ALPHA + (GODMODE_MAX_ALPHA - GODMODE_MIN_ALPHA) * pulseFactor;
            ...
            // --- Skin Data ---
            const skin = p.skinData || { bodyType: "single", bodyColor: p.bodyColor || "white", patternColors: [], trailEffect: "none" };
            const patternColors = skin.patternColors && skin.patternColors.length > 0 ? skin.patternColors : [skin.bodyColor];
            ...
            for (let i = p.trail.length - 1; i >= startIndex; i -= segmentSkipFactor) {
              ...
              if (skin.bodyType === "pattern" && patternColors.length > 0) {
                ...
                const patternIndex = Math.floor(drawnIndex / segmentSkipFactor) % patternColors.length;
              ...
              if (godModeGlowAlpha > 0.01) {
                  ...
                  WebGLUtils.scaleMatrix(modelMatrix, modelMatrix, [segmentRadius + GODMODE_GLOW_OFFSET, segmentRadius + GODMODE_GLOW_OFFSET]);
                  ...
                  gl.uniform4f(colorProgram.uniformLocations.color, GODMODE_GLOW_COLOR[0], GODMODE_GLOW_COLOR[1], GODMODE_GLOW_COLOR[2], godModeGlowAlpha);
                  gl.drawArrays(gl.TRIANGLE_FAN, 0, 4);
              }
              ...
              if (i >= startIndex && particleSpawnCounter >= particleSpawnThreshold && !p.isFrozen && typeof maybeSpawnParticle === 'function') {
            ...
            if (godModeGlowAlpha > 0.01) {
                ...
                WebGLUtils.scaleMatrix(modelMatrix, modelMatrix, [headRadius + GODMODE_GLOW_OFFSET, headRadius + GODMODE_GLOW_OFFSET]);
            ...
            if (!p.isFrozen) {
              ...
              const leftEyeCenterX = p.x + forwardX * eyeForwardOffset + sidewaysX * eyeSidewaysOffset;
              const leftEyeCenterY = p.y + forwardY * eyeForwardOffset + sidewaysY * eyeSidewaysOffset;
              const rightEyeCenterX = p.x + forwardX * eyeForwardOffset - sidewaysX * eyeSidewaysOffset;
              const rightEyeCenterY = p.y + forwardY * eyeForwardOffset - sidewaysY * eyeSidewaysOffset;

              const whiteR = 1.0, whiteG = 1.0, whiteB = 1.0;
              const blackR = 0.0, blackG = 0.0, blackB = 0.0;
              ...
              WebGLUtils.identityMatrix(modelMatrix);
              WebGLUtils.translateMatrix(modelMatrix, modelMatrix, [rightEyeCenterX, rightEyeCenterY]);
              WebGLUtils.scaleMatrix(modelMatrix, modelMatrix, [outerEyeRadius, outerEyeRadius]);
    ...
    drawTextOverlay(textOverlayCtx, textOverlayCanvas, gameState, cameraX, cameraY, zoomLevel, localMyId, selectedPlayerId);

    // --- Draw Minimap ---
    // This draws the minimap elements (background, border, dots, view rect)
    drawMinimapWebGL(gl, canvas, shaderPrograms, buffers, gameState, localMyId, cameraX, cameraY, zoomLevel, worldWidth, worldHeight, mapShape, mapRadius);
