...
const WebGLUtils = (() => {
    ...
    function initWebGL(canvasElement) {
      ...
      if (!gl) {
        console.error(
          "WebGL not supported or unable to initialize. The game cannot start."
        );
        alert(
          "WebGL is not supported by your browser. The game cannot start."
        );
        return false;
      }
      ...
      // 1. Color Shader
      const colorProgram = initShaderProgramInternal(
        vertexShaderSource,
      ...
      // 2. Texture Shader
      const texProgram = initShaderProgramInternal(
      ...
      // 3. Grid Shader
      const gridProgram = initShaderProgramInternal(
    ...
    function drawQuad(x, y, width, height, colorArray) {
      ...
      gl.uniformMatrix4fv(
        programInfo.uniformLocations.mvpMatrix, false, mvpMatrix
      );
      gl.uniform4f(
