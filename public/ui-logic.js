...
let gameContainer, gameOverOverlay, gameOverText, replayButton, backToMenuButton, startMenuOverlay, welcomeMessage, modeSelector, tempForm, loginForm, createForm, soundForm, skinForm, tempModeBtn, loginModeBtn, createModeBtn, skinModeBtn, customSoundsBtn, ghostModeBtn, logoutBtn, deleteAccountBtn, backModeBtn1, backModeBtn2, backModeBtn3, backModeBtn4, backModeBtn6, loginUsernameInput, loginPasswordInput, createUsernameInput, createPasswordInput, createHeadColorInput, createBodyColorInput, startButton, loginButton, createButton, saveSkinButton, clearSoundsButton, leaderboardPanel, leaderboardToggleBtn, showLeaderboardBtn, leaderboardTable, leaderboardContent, adminMessageOverlay, adminLink, notificationContainer, minimapCanvas, exitGhostButton, ghostShortcutGuide, creditsHelpBtn, creditsHelpPanel, closeCreditsHelpBtn;
// Skin form elements
let skinBodyTypeRadios, skinSingleColorSection, skinHeadColorInput, skinBodyColorInput, skinPatternSection, patternColorPickersContainer, addPatternColorBtn, removePatternColorBtn, trailEffectSelect;
...
function initializeUIElements() {
  ...
  patternColorPickersContainer = document.getElementById(
    "patternColorPickers"
  );
...
async function initializeI18n() {
  if (!i18next || !i18nextHttpBackend || !i18nextBrowserLanguageDetector) {
    ...
    errorDiv.innerHTML = '<h2 style="color: red;">Critical Error: Language library failed to load.</h2>';
    return false;
  }
  try {
    await i18next
    ...
    i18next.on('initialized', (options) => {
        console.log("[i18n] Event 'initialized' fired. Loaded languages:", Object.keys(i18next.services.resourceStore.data || {}));
...
function updateContent() {
  ...
  document.querySelectorAll("[data-i18n]").forEach(el => {
    ...
    try {
        if (key.startsWith("[")) { // Attribute translation: [attr]key
        ...
        } else if (key.startsWith("[html]")) { // HTML content: [html]key
        ...
        } else { // Default: textContent
            translation = i18next.t(key, { ...options, defaultValue: `[[${key}]]` });
            if (translation !== `[[${key}]]`) { // Avoid replacing if key not found
...
function enforceLightColor(inputElem, defaultColor) {
...
function applySkinDataToForm() {
  if (!currentSkinData) {
    console.warn(
      "applySkinDataToForm called with null currentSkinData, resetting to default."
    );
    currentSkinData = {
      bodyType: "single",
  ...
  if (skinHeadColorInput)
    skinHeadColorInput.value = currentSkinData.headColor || "#ff0000";
  ...
  if (radioToCheck) radioToCheck.checked = true;
    ...
    const singleRadio = document.querySelector(
      'input[name="bodyType"][value="single"]'
    );
    if (singleRadio) singleRadio.checked = true;
    currentSkinData.bodyType = "single";
  }

  if (skinBodyColorInput)
    skinBodyColorInput.value = currentSkinData.bodyColor || "#ffff00";

  if (trailEffectSelect) {
    const validOption = Array.from(trailEffectSelect.options).some(
      (opt) => opt.value === currentSkinData.trailEffect
    );
    trailEffectSelect.value = validOption ?
      currentSkinData.trailEffect :
      "none";
    if (!validOption) currentSkinData.trailEffect = "none";
  }

  const colors = Array.isArray(currentSkinData.patternColors) ?
    currentSkinData.patternColors :
    [];
...
function toggleSkinSections() {
  ...
  skinSingleColorSection.style.display = selectedType === "single" ?
    "block" :
    "none";
  skinPatternSection.style.display = selectedType === "pattern" ?
    "block" :
    "none";
}
...
function getPatternColorsFromPickers() {
  if (!patternColorPickersContainer) return [];
  return Array.from(
    patternColorPickersContainer.querySelectorAll('input[type="color"]')
...
function handleMenuKeyDown(event) {
  ...
  if (
    ...
    ((document.activeElement.tagName === "INPUT" &&
      ...
      (document.activeElement.tagName === "INPUT" &&
       document.activeElement.type === "password")) // Include password fields
  ) {
    if (event.key === "Enter") activateFocusedButton(); // Allow Enter in text fields to submit form
    return; // Don't interfere with typing
  }
  if (
    creditsHelpPanel && creditsHelpPanel.classList.contains("visible")
  ) {
  ...
  switch (event.key) {
    ...
    case "Escape":
      ...
      else if (creditsHelpPanel.classList.contains("visible"))
        closeCreditsHelpPanel();
      break;
  }
...
function toggleZeroGravity() {
  ...
  if (zeroGravityActive) {
    ...
    const elementsToFloat = innerContainer.querySelectorAll(
      ".modeSelector button, .modeForm"
    );
    ...
    const elementsToReset = innerContainer.querySelectorAll(
      ".modeSelector button, .modeForm"
    );
...
function setupEventListeners() {
  ...
  createBodyColorInput.addEventListener("change", () => {
    enforceLightColor(createBodyColorInput, "#ffff00");
    ...
    fetch("/login", {
      ...
      .then((response) =>
        response.ok ?
          ...
          response.json().then((errData) => { // Expect JSON error too
    ...
    fetch("/createAccount", {
      ...
      .then((response) =>
        response.ok ?
          ...
          response.json().then((errData) => { // Expect JSON error
            throw new Error(t(errData.messageKey, errData.messageOptions || {})); // Translate server error key
          })
      )
      .then((data) => {
        if (data.success) {
          ...
          accountData = {
            username, headColor, bodyColor,
            skinData: {
              bodyType: "single", bodyColor: bodyColor,
              patternColors: [], trailEffect: "none",
            },
          };
  ...
  saveSkinButton.addEventListener("click", () => {
    ...
    if (loggedInUsername) {
      ...
      fetch("/updateSkin", {
        ...
        .then((response) => {
          if (!response.ok) {
            return response.json().then((errData) => { // Expect JSON error
  ...
  replayButton.addEventListener("click", () => {
    gameOverOverlay.style.display = "none";
    gameOverOverlay.style.opacity = "0";
    if (typeof Game !== "undefined" && typeof Game.stopGame === "function")
      Game.stopGame();
    gameHasStarted = false;
    if (socket && socket.connected) socket.disconnect();
    socket = null;
    ...
    adminLink.style.display = isAdmin ? "block" : "none";
    showForm(null);
    if (wasLoggedInBeforeDisconnect && loggedInUsername)
      updateLoginStateUI();
    ...
    if (loggedInUsername && accountData) {
      ...
      const tempSkinData = {
        bodyType: document.querySelector('input[name="bodyType"]:checked')
          .value,
        headColor: skinHeadColorInput.value,
        bodyColor: skinBodyColorInput.value,
        patternColors: getPatternColorsFromPickers(),
...
function updateLoginStateUI() {
  if (
    !leaderboardPanel || !welcomeMessage || !logoutBtn || !loginModeBtn ||
    !createModeBtn || !tempModeBtn || !showLeaderboardBtn ||
    !customSoundsBtn || !ghostModeBtn || !adminLink || !skinModeBtn ||
    !deleteAccountBtn
  )
    return;
  ...
  if (displayUsername) {
    ...
    welcomeMessage.textContent = t('ui.welcomeMessage', { username: displayUsername, context: isAdmin ? 'admin' : '' });
...
function initSocket(joinData) {
  ...
  socket.on("connect", () => {
    ...
    if (
      ...
      typeof Game.initializeAndStart === "function"
    ) {
      Game.initializeAndStart(socket, myId, textOverlayCanvas);
      gameHasStarted = true;
      exitGhostButton.style.display = finalJoinData.mode === "ghost" ?
        "block" :
        "none";
      ghostShortcutGuide.style.display = finalJoinData.mode === "ghost" ?
        "block" :
        "none";
  ...
  socket.on("configUpdate", (data) => {
    if (
      typeof Game !== "undefined" && typeof Game.updateConfig === "function"
    )
...
function updateLeaderboard(data) {
  ...
  if (!data || data.length === 0) {
    ...
    leaderboardContent.innerHTML =
      `<tr><td colspan="6" class="leaderboard-message">${t('leaderboard.noPlayers')}</td></tr>`;
    return;
  }
...
function showGameOverOverlay(reasonKey, reasonOptions, kill, finalSize) {
  ...
  if (!gameOverOverlay || !gameOverText) {
    ...
    alert(`${t(reasonKey, reasonOptions)} - ${t('gameOver.finalSizeLabel', { size: finalSize || 0 })}`);
    return;
  }
  console.log(
    `Showing Game Over: ReasonKey='${reasonKey}', Kill=${kill}, Size=${finalSize}`
  );
